...
int write_ektl_fw_page(unsigned char *p_ektl_fw_page_buf, size_t ektl_fw_page_buf_size)
{
    ...
    // Get Frame Count
    frame_count = (ektl_fw_page_buf_size / ELAN_HID_PAGE_FRAME_SIZE) +
                  ((ektl_fw_page_buf_size % ELAN_HID_PAGE_FRAME_SIZE) != 0);

    // Write eKTL FW Page Data with Frames
    for(frame_index = 0; frame_index < frame_count; frame_index++)
    {
        if((frame_index == (frame_count - 1)) && ((ektl_fw_page_buf_size % ELAN_HID_PAGE_FRAME_SIZE) > 0)) // The Last Frame
            frame_data_len = ektl_fw_page_buf_size % ELAN_HID_PAGE_FRAME_SIZE;
        else
            frame_data_len = ELAN_HID_PAGE_FRAME_SIZE;
...
int gen8_read_remark_id(unsigned char *p_gen8_remark_id_buf, size_t gen8_remark_id_buf_size, bool recovery)
{
    ...
    unsigned char gen8_remark_id_data[ELAN_GEN8_REMARK_ID_LEN] = {0},
                  ...
                  gen8_remark_id_address_set_index = 0,
                  data_index = 0;
    ...
    gen8_remark_id_address_set_index = 0xFF - LOW_BYTE_OF_UINT(rom_data);
    DEBUG_PRINTF("%s: Gen8 Address Set %d.\r\n", __func__, gen8_remark_id_address_set_index);
    ...
    /* [Note] Paul @ 202312/19
     * Address Set Index starts from 1.
     * It means if Address Set Index is 1, then Remark ID Index is 0.
     * $(remark_id_index) = $(address_set_index) - 1
     */
    gen8_remark_id_index = gen8_remark_id_address_set_index - 1;
    DEBUG_PRINTF("%s: Gen8 Remark ID Index: %d.\r\n", __func__, gen8_remark_id_index);

    // Remark ID Address
    gen8_remark_id_address = gen8_remark_id_address_set[gen8_remark_id_index];
    DEBUG_PRINTF("%s: Gen8 Remark ID Address %d: 0x%08x.\r\n", __func__, gen8_remark_id_index, gen8_remark_id_address);

    // Read ROM Data from Remark ID Address $(gen8_remark_id_address)
    for(data_index = 0; data_index < ELAN_GEN8_REMARK_ID_LEN; data_index++)
    {
        ...
        gen8_remark_id_data[data_index] = LOW_BYTE_OF_UINT(rom_data);
...
int gen8_read_memory_page(unsigned short mem_page_address, unsigned short mem_page_size, unsigned char *p_mem_page_buf, size_t mem_page_buf_size)
{
    ...
    unsigned char gen8_mem_page_buf[ELAN_GEN8_MEMORY_PAGE_SIZE] = {0},
                  data_buf[ELAN_HID_DATA_BUFFER_SIZE] = {0},
                  recv_frame_index = 0;
    ...
    // Receive Page Data
    page_frame_count = (mem_page_size / ELAN_HID_READ_PAGE_FRAME_SIZE) + \
                       ((mem_page_size % ELAN_HID_READ_PAGE_FRAME_SIZE) != 0);
    for(page_frame_index = 0; page_frame_index < page_frame_count; page_frame_index++)
    {
        ...
        // Data Length
        if((page_frame_index == (page_frame_count - 1)) && ((mem_page_size % ELAN_HID_READ_PAGE_FRAME_SIZE) != 0)) // Last Frame
            page_frame_data_len = mem_page_size % ELAN_HID_READ_PAGE_FRAME_SIZE;
        else // (page_frame_index != (page_frame_count -1)) || ((ELAN_FIRMWARE_PAGE_DATA_SIZE % ELAN_HID_READ_PAGE_FRAME_SIZE) == 0)
            page_frame_data_len = ELAN_HID_READ_PAGE_FRAME_SIZE;
        ...
        // Verify Received Frame with Frame Index
        recv_frame_index = data_buf[1];
        DEBUG_PRINTF("%s: [%d] Received Frame Index = 0x%x(%d).\r\n", __func__, page_frame_index, data_buf[1], recv_frame_index);
        if(recv_frame_index != page_frame_index)
        {
            ERROR_PRINTF("%s: [%d] Invalid Received Frame Data = {%02x %02x %02x ...}.\r\n", \
                          __func__, page_frame_index, data_buf[0], data_buf[1], data_buf[2]);
