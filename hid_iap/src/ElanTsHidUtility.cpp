...
{
    int err = ERR_SUCCESS;
    unsigned char hid_frame_data[ELAN_HID_OUTPUT_BUFFER_SIZE] = {0};

    // Validate Data Length
    if((data_len == 0) || (data_len > ELAN_HID_PAGE_FRAME_SIZE))
    {
    ...
    if((frame_buf_size < data_len) || (frame_buf_size > ELAN_HID_OUTPUT_BUFFER_SIZE))
    {
