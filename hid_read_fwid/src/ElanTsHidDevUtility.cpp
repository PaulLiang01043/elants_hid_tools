/** @file
  ...
  Implementation of HID Device Utility for Elan HID (I2C-HID / SPI-HID) Touchscreen.
...
int validate_elan_hid_device(struct hidraw_devinfo *p_hid_dev_info, size_t dev_info_size, int pid, bool silent_mode)
{
    ...
    for(index = 0; index < DEV_INFO_SET_MAX; index++)
    {
        ...
        if(((p_hid_dev_info[index].bustype == BUS_I2C) || (p_hid_dev_info[index].bustype == BUS_SPI) || (p_hid_dev_info[index].bustype == BUS_PCI) /* THC IC */ ) && \
            (p_hid_dev_info[index].vendor == ELAN_HID_VID) /* nVID = usb_dev_desc.idVendor = ELAN_HID_VID */ && \
           ((p_hid_dev_info[index].product == pid) || (p_hid_dev_info[index].product == ELAN_HID_RECOVERY_PID)))
        {
            if(silent_mode == false)
