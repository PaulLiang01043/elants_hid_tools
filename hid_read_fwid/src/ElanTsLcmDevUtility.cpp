/** @file
  ...
  Implementation of LCM Device Utility for Elan HID (I2C-HID / SPI-HID) Touchscreen.
        ...
        if(strcmp(p_dev_info[index].panel_info, target_panel_info) == 0)
        {
            if(system == CHROME)
            {
                ...
                if(p_dev_info[index].chrome_fwid == 0x0) // NULL FWID
                {
                    ...
                    err = ERR_DATA_NOT_FOUND; // Not Found
                }
                else // Valid FWID
                {
                    *p_fwid = p_dev_info[index].chrome_fwid;
                    err = ERR_SUCCESS; // Found
                }
                break;
            }
